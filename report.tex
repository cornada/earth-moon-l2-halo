...
\pagestyle{fancy}
\fancyhead[L]{Earth--Moon $L_2$ orbit stabilization}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
...
\title{\vspace{-1.5cm}
\textbf{Stabilization of Periodic Orbits near the Earth--Moon $L_2$ Libration Point} \\
...
The collinear libration point $L_2$ of the Earth--Moon system sits at $x_{L_2} \approx 1.1557$ in non-dimensional CR3BP units ($\approx\SI{0.168}{}$ away from the Moon on the anti-Earth side). It is one of the most operationally relevant parking points in cislunar space, hosting mission concepts for farside radio astronomy, nearside-farside communication relays (Queqiao class), and lunar-surface staging for Lunar Gateway / Artemis architecture.

The Earth--Moon system is the workhorse of halo-orbit theory: every major mission concept for the Moon from Apollo through the Lunar Gateway uses $L_1/L_2$ orbits at some stage. Its mass ratio $\mu = 1.215\times 10^{-2}$ is four orders of magnitude larger than Sun--Earth, elevating $L_2$ onto a much steeper saddle: the dominant monodromy eigenvalue reaches $\lambda_u \approx 1.45\times 10^3$ per revolution for the planar Lyapunov orbit, meaning that any navigation error is amplified by three orders of magnitude over one 3-CR3BP-unit period ($\approx\SI{27.32}{days}$ in physical time). Without active stabilization, a spacecraft drifts off the reference orbit within two to three revolutions. The correction strategy therefore is not a cosmetic detail; it is the mission.
...
\paragraph{Earth--Moon specifics.}
The Earth--Moon system is the most-studied three-body problem in astrodynamics since Apollo. Folta and Woodard~\cite{anderson2011} analysed the NRHO and DRO families for the Lunar Gateway/Artemis programme, and Bollt and Meiss~\cite{bollt1995} studied chaotic transport via Cantori near the outer resonances. The large mass ratio $\mu \sim 10^{-3}$ makes many linearization-based approximations degrade faster than in Sun--Earth, motivating fully numerical methods such as the one presented here.
...
Let $m_1$ (Earth) and $m_2$ (Moon) orbit their common barycenter on circular orbits with mutual separation normalized to unity. Defining $\mu = m_2/(m_1+m_2)$, the CR3BP rotating-frame equations for a massless spacecraft $(x,y,z)$ read
\begin{equation}
\label{eq:cr3bp}
\boxed{\;
\begin{aligned}
...
The five equilibria of (\ref{eq:cr3bp}) satisfy $\nabla \Omega = 0$. The three collinear points $L_{1,2,3}$ lie on the $x$-axis and are solutions of the fifth-order polynomial $\Omega_x(x,0,0) = 0$. For $\mu = 1.215\times 10^{-2}$ (Earth--Moon),
\begin{equation}
x_{L_1} = 0.932032, \quad x_{L_2} = 1.155682, \quad x_{L_3} = -1.000397.
\end{equation}
...
\end{equation}
with $\gamma = x_{L_2} - (1-\mu) = 0.1679$ and $c_2 \approx 3.192$. Solving yields
\begin{equation}
\lambda = \pm \sqrt{5.534} \approx \pm 2.35, \qquad
\lambda = \pm i \sqrt{3.910} \approx \pm 1.977\,i,
\end{equation}
i.e.\ a saddle eigenvalue $\lambda_{\text{saddle}} = 2.35$ (exponential instability) and a planar oscillation frequency $\omega_p = 2.160$ (period $T_p = 2\pi/\omega_p \approx 3.373$, matching the nonlinear Lyapunov period to $10^{-4}$). The vertical block gives $\omega_v = \sqrt{c_2} \approx 1.787$, and the non-resonant ratio $\omega_p/\omega_v \neq 1$ explains the absence of small-amplitude halo orbits: nonlinear frequency shifts must compensate for the linear mismatch.
...
\[
x_0 = x_{L_2} + 2.0\times 10^{-3} = 1.175682, \quad \dot y_0 = -1.24370\times 10^{-2}, \quad T = 3.373431,
\]
...
\[
\{\lambda_u, \lambda_s\} = \{1452.40, 5.71\times 10^{-4}\},
...
\caption{\textbf{Hill region and Lyapunov orbit.} The red shaded region is forbidden (motion requires $v^2 < 0$); its boundary is the zero-velocity curve for the orbit's Jacobi constant $C \approx 3.04$. $L_1$ (inner gateway), $L_2$ (outer gateway), $L_3$ (opposite side of Earth) are marked. The tiny Lyapunov orbit at $L_2$ is invisible at this scale, emphasizing the fundamental paradox of libration-point missions: the orbit occupies $2\times 10^{-3}$ of the system scale but governs access to an entire outer-planet corridor.}\label{fig:hill}
\end{figure}

\textbf{Physical meaning:} The Hill region geometry shows that $L_2$ is the natural ``gate'' separating the Earth-Moon neighborhood from escape. The narrow isthmus connecting Moon to the outer forbidden region passes through $L_1$ and $L_2$; closing it requires raising $C$ (cooling the orbit); opening it fully requires lowering $C$ (heating). At the orbit's $C$ we sit right at the gate, which is why small energy perturbations can either re-enter the Moon region or escape to $L_3$.
...
\textbf{Insight:} The instability strength $\lambda_u \approx 1452$ is the central quantitative fact of the problem. Compared to Sun--Earth $L_2$ ($\lambda_u \approx 1500$) and Sun--Jupiter $L_2$ ($\lambda_u \approx 1750$), the Earth--Moon $L_2$ sits at the same order of magnitude despite the very different mass ratio; this is because the saddle steepness is set by the logarithmic geometry of the three-body potential, not by $\mu$ alone. Physically this means a $1\,\mathrm{m}$ position error balloons to $\sim 1.5\,\mathrm{km}$ after one $27.32$-day revolution.
...
\begin{table}[H]
\centering
\caption{Halo family at Earth--Moon $L_2$ (selected members).}
\begin{tabular}{@{}r r r r r r@{}}
\toprule
$A_z$ & $x_0 - x_{L_2}$ & $\dot y_0$ & $T$ & $|\lambda_u|$ & $C$ \\
\midrule
0.0050 & $-3.548\times 10^{-2}$ & $+1.766\times 10^{-1}$ & 3.4151 & 1208 & 3.147 \\
0.0080 & $-3.576\times 10^{-2}$ & $+1.774\times 10^{-1}$ & 3.4145 & 1201 & 3.146 \\
0.0120 & $-3.634\times 10^{-2}$ & $+1.791\times 10^{-1}$ & 3.4133 & 1187 & 3.144 \\
0.0150 & $-3.693\times 10^{-2}$ & $+1.808\times 10^{-1}$ & 3.4120 & 1173 & 3.142 \\
0.0180 & $-3.771\times 10^{-2}$ & $+1.830\times 10^{-1}$ & 3.4105 & 1156 & 3.140 \\
0.0220 & $-3.822\times 10^{-2}$ & $+1.845\times 10^{-1}$ & 3.4091 & 1128 & 3.138 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Representative halo at $A_z = 0.012$.} 3D view showing the orbit lying on the Moon side of $L_2$ (note $x_0 - x_{L_2} < 0$), tilted out of the ecliptic by $z_{\max} = A_z$. The starting point (green star) is on the $y = 0$ symmetry plane.}\label{fig:halo3d}
\end{figure}
...
in the Earth--Moon $L_2$ context specifically. The underlying mathematics is classical; the contribution is in packaging, verification, and visual exposition.
...
\textbf{Live demo:} \href{https://cornada.github.io/earth-moon-l2-halo/}{\texttt{cornada.github.io/earth-moon-l2-halo}}\\
\textbf{Source:} \href{https://github.com/cornada/earth-moon-l2-halo}{\texttt{github.com/cornada/earth-moon-l2-halo}}
}}
\end{center}
...
You are looking into the \emph{rotating reference frame} locked to the Earth--Moon barycenter: both primaries sit still in this frame while the entire frame spins about the system center of mass at the Moon orbital rate (one revolution every $\SI{27.32}{days}$). Anything that is not gravitationally bound to a primary will appear to ``drift'' in this view as Coriolis and centrifugal pseudo-forces act on it.
...
\midrule
Large yellow sphere + glow, labeled ``Earth'' & Earth, $m_1 \approx \SI{1.99e30}{kg}$, 99.905\% of system mass. Radius enlarged $\sim 50\times$ for visibility. & $(x,y,z) = (-\mu, 0, 0) \approx (-\SI{0.001}{}, 0, 0)$ \\
Brown sphere labeled ``Moon'' & Moon, $m_2 \approx \SI{1.9e27}{kg}$, 0.095\% of system mass. Radius enlarged $\sim 100\times$; the \emph{real} Moon radius in these units is $\sim 10^{-4}$ (the size of a pixel on screen). & $(1-\mu, 0, 0) \approx (0.999, 0, 0)$ \\
Small white dot labeled ``L1'' & Interior Lagrange point $L_1$, the saddle-type equilibrium between Earth and Moon. The gateway for mass transfer inward. & $(0.9324, 0, 0)$ \\
Small white dot labeled ``L2'' & Exterior Lagrange point $L_2$, the saddle-type equilibrium on the anti-Earth side of Moon. The focal point of this entire report. & $(1.1557, 0, 0)$ \\
...
\textbf{Scale disclaimer.} The demo lies about sizes for pedagogical clarity. In a faithful-scale rendering, Earth and Moon would be single pixels and the halo orbit would be a barely-visible filament. The inflation factor is about $50\times$ for Earth, $100\times$ for Moon, and the Lagrange-point markers are symbolic.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Layers.}
    Toggle $W^u$, $W^s$, halo-family ensemble, Earth/Moon/Lagrange markers, cost colormap. Turning off cost colors reveals the pure geometric shape of the orbit (solid blue); turning off $W^{u,s}$ isolates the periodic orbit from its invariant manifolds.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Drag the mouse to rotate. Use the wheel to zoom. Notice the perspective scales: Earth is far to the left, Moon is a small brown ball mid-scene, and the halo is a loop around a white marker (L2).
...
\end{enumerate}
\textbf{Physical takeaway:} a halo orbit is a closed 3D curve, not a 2D loop. The rotating frame lets you picture it as stationary in space relative to Earth and Moon.
...
\end{enumerate}
\textbf{Physical takeaway:} the invariant manifolds are the ``highway network'' of the CR3BP. An arriving spacecraft wanting to enter the halo will find its cheapest path along the green tube. An escaping one (or a comet leaving Moon's vicinity) naturally finds itself on the red tube. These tubes extend to neighbouring Lagrange points (L1 and L3) and can carry ballistic trajectories between them \emph{at zero fuel cost}. The Genesis and ARTEMIS missions were designed along such tubes.
...
Earth, Moon (spheres)        & Two gravitating primaries               & $m_1, m_2$ with $\mu = m_2/(m_1+m_2)$ in CR3BP equations (\ref{eq:cr3bp}) \\
...
Rotating frame                & Co-rotating with Earth--Moon           & Transformation $(x,y,z)_{\text{inertial}} \to (x,y,z)_{\text{rot}}$ via $R_z(\omega_J t)$ \\
...
$L_1, L_2, L_3$ & The three collinear Lagrange points. $L_2$ lies on the anti-Earth side of Moon. \\
...
$\mu$ & Mass ratio $m_2/(m_1+m_2)$. For Earth--Moon, $\mu = 1.215\times 10^{-2}$. \\
...
AU & Astronomical unit. $\SI{1.496e11}{m}$. Earth--Moon non-dimensionalization uses the Earth--Moon separation $\approx \SI{384000}{km}$ as the length unit. \\
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Ephemeris-accurate extension.}
    Move from CR3BP to the true-ephemeris four-body model (Sun + Earth + Moon + S/C) using JPL ephemerides via SPICE. Compare CR3BP with ephemeris; quantify lunar-eccentricity ($e_\text{Moon}=0.0549$) correction to the stationary Floquet law.
...
\begin{enumerate}[resume,leftmargin=*]
  ...
  \item \textbf{Time-varying Jacobi structure.}
    Study the non-autonomous equivalent with $\mu(t)$ varying (to model the eccentricity of Moon's orbit); this is the Elliptic RTBP. Quantify how the correction-law sensitivity degrades with orbital eccentricity $e_J \approx 0.049$.
\end{enumerate}
...
R.~L.~Anderson and M.~W.~Lo, Role of invariant manifolds in low-thrust trajectory design, \textit{J. Guidance, Control, and Dynamics} \textbf{32}(6) (2009), 1921--1930; Moon applications in subsequent JPL tech reports.
